...
module MakeNVec (X : sig
  ...
  let of_list l =
    if List.length l = n then Array.of_list l
    else failwith "MakeNVec.of_list: length don't match!"
...
module Vec3 : Vec = MakeNVec (struct
  let n = 3
end)
...
let cross v1 v2 =
  Vec3.of_list
    [
      (Vec3.nth v1 1 *. Vec3.nth v2 2) -. (Vec3.nth v1 2 *. Vec3.nth v2 1);
...
(* these are basically hardcoded matrix multiplication *)
let r_alpha alpha v3 =
  let x =
    Vec3.dot (Vec3.of_list [ Float.cos (-.alpha); Float.sin alpha; 0. ]) v3
  and y =
    Vec3.dot (Vec3.of_list [ Float.sin (-.alpha); Float.cos (-.alpha); 0. ]) v3
  and z = Vec3.dot (Vec3.of_list [ 0.; 0.; 1. ]) v3 in
  Vec3.of_list [ x; y; z ]

(* rotation along y-axis for beta angle *)
let r_beta beta v3 =
  let x = Vec3.dot (Vec3.of_list [ Float.cos beta; 0.; Float.sin beta ]) v3
  and y = Vec3.dot (Vec3.of_list [ 0.; 1.; 0. ]) v3
  and z = Vec3.dot (Vec3.of_list [ -.Float.sin beta; 0.; Float.cos beta ]) v3 in
  Vec3.of_list [ x; y; z ]
...
let mapSphereSample v3 ~alpha ~beta =
  v3 |> r_alpha alpha |> r_beta beta
  |> Vec3.( + ) (Vec3.of_list [ 0.; 0.; 1. ])
  |> sProj ~z_c:1.
