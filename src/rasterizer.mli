...
val getImage :
  ?camera_offset:float ->
  (* camera height, only applies in orthogonal view*)
  ?view_direction:Vec3.t ->
  (* camera forward direction *)
  ?img_w:int ->
  (* image width *)
  ?view_size:int ->
  (* the planar view size, e.g., in planar view we sees the [-view_size, view_size] x [-view_size, view_size] area *)
  ?plane_bd:int ->
  (* the orthogonal view boudnary. This is to prevent the planer image occupies the whole orthogonal view *)
  ?half_edge_length:int ->
  (* the length of grid. The grid sit in the [-half_edge, half_edge] x [-half_edge, half_edge] area *)
  ?line_w:float ->
  (* width of grid lines *)
  ?grid_size:int ->
  (* number of lines. We will have grid_size grids and grid_size+1 vertical/horizontal corresponding lines *)
  ?sampling_n:int ->
  (* number of samples per pixel *)
  render_mode ->
  (* output viewmode, this is non-parameterized bcs of warning 16 [unerasable-optional-argument] *)
  alpha:Degree.t ->
  (* rotation along z axis in degree*)
  beta:Degree.t ->
